...
export async function ask<S extends z.ZodType>(opts: {
  ...
  const tools: Anthropic.Beta.BetaToolUnion[] = [
    { name: TOOL, description: "Entrega la respuesta final. Llámala una sola vez, al final.", strict: true, eager_input_streaming: true, input_schema: toolSchema(opts.schema) },
  ];
  if (opts.webSearch) {
    tools.unshift({
  ...
  for (let turn = 0; turn < 4; turn++) {
    ...
    try {
      ...
      if (e instanceof Anthropic.RateLimitError) throw new AIError("Claude está muy solicitado ahora mismo. Intenta en un minuto.");
      if (e instanceof Anthropic.APIConnectionTimeoutError) throw new AIError("Claude tardó demasiado. Intenta de nuevo.");
      if (e instanceof Anthropic.APIError) throw new AIError(`Claude no pudo responder (${e.status ?? "sin conexión"}). Intenta de nuevo.`);
      throw e;
    }
